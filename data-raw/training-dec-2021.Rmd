...
lines_all %>%
  sf::st_drop_geometry() %>% 
  summarise(mean_pct_cycling = mean(pcycle), sd = sd(pcycle))
```

That tells that the the average value of % cycling is 6%.
...
See the books Reproducible Road Safety Research with R, Geocomutation with R and R for Data Science for more for developing your R code writing skills.
...
lines_to_plot_baseline = lines_all %>% 
  transmute(Percent = pcycle * 100) %>% 
  arrange(desc(Percent))


# plot(lines_to_plot_baseline, lwd = lwd, breaks =  breaks)
# plot(lines_all["pcycle_dutch"], lwd = lines_all$all / mean(lines_all$all), breaks = c(0, 5, 10, 20, 50) / 100)

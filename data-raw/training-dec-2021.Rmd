...
```{r}
zones_leeds = zones_all %>% filter(lad_name == "Leeds")
```
...
buildings_nyorks = osmextract::oe_get("west yorkshire", query = q)
```
...
```{r}
leeds_clockboard = zonebuilder::zb_zone("leeds", n_circles = 3)
leeds_north = leeds_clockboard %>% 
  filter(circle_id == 1 | segment_id == 12)
plot(leeds_clockboard$geometry)
plot(leeds_north, add = TRUE)
```

```{r, eval=FALSE}
buildings_leeds = buildings_nyorks[york_north, , op = sf::st_within] 
```
...
```{r, eval=FALSE, echo=FALSE}
sf::write_sf(buildings_leeds, "buildings_leeds_north.geojson")
piggyback::pb_upload("buildings_leeds_north.geojson")
piggyback::pb_download_url("buildings_leeds_north.geojson")
```

```{r}
u = "https://github.com/ITSLeeds/pct/releases/download/v0.9.4/buildings_leeds_north.geojson"
buildings_leeds = sf::read_sf(u)
```
...
```{r}
centroids = sf::st_centroid(zones_leeds)
centroids_central = centroids[leeds_north, ]
zones_central = zones_leeds %>% filter(geo_code %in% centroids_central$geo_code)
```

We can then subset the OD pairs, represented as routes in the `routes_towns_active` object, as follows:

```{r}
routes_leeds = routes_towns_active %>% 
...
```{r}
sum(routes_leeds$all) # 30k commuters
zones_central %>% select(foot) %>% plot(reset = FALSE)
plot(routes_leeds$geometry, add = TRUE, col = "white", lwd = 3)
plot(buildings_leeds$geometry, add = TRUE)
```



```{r}
od_leeds_abstr = routes_leeds %>% 
  transmute(
    o = geo_code1,
    d = geo_code2,
...
output_sf = abstr::ab_scenario(
  od = od_leeds_abstr,
  zones = zones_leeds,
  origin_buildings = buildings_leeds,
  destination_buildings = buildings_leeds,
  pop_var = 3,
  time_fun = ab_time_normal,
  output = "sf"
)
# plot(output_sf$geometry)
output_sf %>% sample_n(100) %>% mapview::mapview()
```

We can then convert the result to a list format and save the data as a `json` file:

```{r, eval=FALSE, echo=FALSE}
# output_json = abstr::ab_json(output_sf, scenario_name = "go_active_leeds", time_fun = abstr::ab_time_normal(7.5)) # fails but shouldn't, right?
```


```{r}
output_json = abstr::ab_json(output_sf, scenario_name = "go_active_leeds")
abstr::ab_save(output_json, f = "go_active_leeds.json")
system("head go_active_leeds.json")
```

```{r, echo=FALSE, eval=FALSE}
piggyback::pb_upload("go_active_leeds.json")
```

## Importing scenario.json files and using A/B Street for active travel planning

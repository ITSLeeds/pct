...
<!-- It depends -->
- Bonus: find the areas that would benefit from improved cycling provision on Clifton bridge, 1 km northwest from central York over the River Ouse (see result in Figure \@ref(fig:clifton))^[
Hint: Select the Fast Routes option from the Cycling Flows menu, zoom into the Bridge, and then click on the Freeze Lines checkbox to prevent the selected routes from moving when you zoom back out.
]
...
- Download the Route network layer in [geojson](https://npttile.vs.mythic-beasts.com/pct-outputs-regional-notR/commute/lsoa/north-yorkshire/rnet_full.geojson) format
- Download the Fast routes in [geojson](https://npttile.vs.mythic-beasts.com/pct-outputs-regional-notR/commute/lsoa/north-yorkshire/rf.geojson) format (warning: this file is nearly 100 MB in size so may take some time to download)
...
You can see how it works by opening the project pctqgis3.qgz in the [pctqgis.zip](https://github.com/ITSLeeds/pct/releases/download/0.8.1/pctqgis.zip) file.
...
```{r, echo=FALSE, eval=FALSE}
cycle_infra = osminfra::oi_get(place = "north yorkshire", infra_type = "cycle_lane")
cycle_infra_projected = sf::st_transform(cycle_infra, 27700)
sf::write_sf(cycle_infra, "pctqgis/cycle_infra.shp")
sf::write_sf(cycle_infra_projected, "pctqgis/cycle_infra_projected.shp")
```

This section assumes you have data on the existing regional cycle network.
Cycle infrastructure can be classified into various categories, such as cycle lanes, traffic free cycleways and shared paths.
For the purposes of this worked example, we extracted on-road cycle lanes, using an OSM definition based on research undertaken at Leeds and at the [University of Heidelberg](http://k1z.blog.uni-heidelberg.de/2020/10/02/how-to-become-ohsome-part-8-complex-analysis-with-the-magical-filter-parameter/).
Using this definition, cycle lanes were extracted from OSM using R.
The data was exported from R into `cycle_infra_projected.shp`, which is provided in the `pctqgis.zip` file mentioned in the previous sections.
...
- Load the `cycle_infra_projected.shp` dataset
- Buffer it to 20 m 
- Clip the route network layer to include only areas outside the buffer

```{r, echo=FALSE}
knitr::include_graphics("https://user-images.githubusercontent.com/52210872/130649292-c59f57a7-a24f-4cbd-8d2d-334834cefbcb.png")
```

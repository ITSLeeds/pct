...
Let's use a remote routing service to convert the straight lines generated in the previous section into routes that could realistically be taken by people cycling.
...
leaflet() %>% 
  addTiles() %>% 
  addPolylines(data = routes_santiago_cs[32, ])
```
...
```{r}
routes = sf::st_sf(
  cbind(sf::st_drop_geometry(routes_santiago_cs),
  sf::st_drop_geometry(desire_lines)),
  geometry = routes_santiago_cs$geometry
)
```

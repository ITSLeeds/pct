...
By default, the function includes intra-zonal flows, but these can be omited as follows (the argument `omit_intrazonal` in `get_od()` does the same thing):
...
Note that the resulting lines contain 50% of the number of lines, but the same number of trips: this is because 2 separate lines between the same zones have been coverted into 1 line representing the combined number of trips in both directions, for each OD pair.
...
Some routes have become shorter, which can be explained by changes in infrastructure since the PCT results were first generated (these will be updated in the Propensity to Cycle Tool online data later in 2019).

We now have everthing needed to estimate cycling uptake for each desire lines on the Isle of Wight (we'll do the calculation on the top 30 by current cycling levels).
...
This work is done by the `overline2()` funtion, as follows:

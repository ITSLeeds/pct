...
zones_all = get_pct_zones("london")
lines_all = get_pct_lines("london")
...
routes_dependent = stplanr::line2route(car_dependent)
routes_dependent_joined = cbind(sf::st_drop_geometry(routes_dependent), sf::st_drop_geometry(car_dependent))
routes_dependent_joined = sf::st_sf(routes_dependent_joined, geometry = routes_dependent$geometry)

pal = colorBin(palette = "RdYlBu", domain = car_dependent$`Percent Active`,
               bins = c(0, 20, 40, 60, 80, 100), reverse = TRUE)
leaflet(data = routes_dependent_joined) %>% 
  addProviderTiles(providers$OpenStreetMap.BlackAndWhite) %>% 

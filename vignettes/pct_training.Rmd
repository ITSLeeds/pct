...
It makes sense to use R for advanced applications using PCT data because the tool was developed in and can be extended with R code.
Furthermore, using open source software with a command-line interface reduces barriers to entry, enabling the development of [open access transport models](https://doi.org/10.1016/j.tranpol.2020.06.015) for more citizen-led and participatory transport planning [@lovelace_open_2020].
...
If you are new to R, it is worth installing R and RStudio before the course.
may be worth brushing-up on your skills.
You can do so by taking a [free online course](https://www.datacamp.com/courses/free-introduction-to-r) or by working through [Chapter 2 of the open source book *Geocomputation with R*](https://geocompr.robinlovelace.net/spatial-class.html).
It may also be worth taking a read of one or more of the following:
...
- Propensity to Cycle Tool: getting and using the data, 11:30 - 12:30

- Propensity to Cycle Tool: build your own scenarios, 13:30 - 14:30
    
# Exercises

## How the PCT works and what you can use it for

- Take a look at the image below from [@woodcock_health_2021a], which builds on an earlier diagram in @lovelace_propensity_2017. Which parts of the process are of most interest for your work?
...
- E3: Use the `plot()` command to visualise where on the Isle of Wight this 'high cycling' zone is (hint: you will need to use the `plot()` function twice, once to plot `z$geometry`, and again with the argument `add = TRUE` and a `col` argument to add the layer on top of the base layer and give it a colour).
...
<!-- From ccag 2021 -->
This advanced section is designed for people with experience with the PCT and cycling uptake estimates who want to learn more about how uptake models work and how to generate new scenarios of change.
Reproducible and open R code will be used to demonstrate the concepts so knowledge of R or other programming languages is recommended but not essential, as there will be conceptual exercises covering the factors linked to mode shift. In it you will:
